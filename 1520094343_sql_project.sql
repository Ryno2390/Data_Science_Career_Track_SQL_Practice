...
SELECT facid,
       ...
       membercost,
       monthlymaintenance,
      (membercost / monthlymaintenance) * 100 AS percent_of_maint
    FROM `Facilities`
...
SELECT facilities.name AS facility_name,
       ...
       ELSE facilities.membercost * bookings.slots END AS facility_cost
    FROM `Bookings` bookings
    JOIN `Facilities` facilities
    ON bookings.facid = facilities.facid
    JOIN `Members` members
...
SELECT sub.*
  ...
  WHERE sub.facility_cost > 30
  ORDER BY sub.facility_cost DESC
...
SELECT sub.*
    FROM (
SELECT sub.facility_name, SUM(sub.revenue) AS total_revenue
    FROM (
          SELECT facilities.name AS facility_name,
                 CASE WHEN members.memid = 0 THEN facilities.guestcost * bookings.slots
